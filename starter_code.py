...
import os
# Added this to support Unicode characters in the terminal output 
import sys
...
def sum_list(numbers):
    ...
    # Checking if numbers is empty is stronger than checking if len(numbers) == 0, because it also handles None
    if not numbers:
        return 0
...
def find_infected_files(directory_path, extension=".encrypted"):
    ...
    # Case 1: It's a file - check if it ends with the target extension
    if os.path.isfile(directory_path):
        if directory_path.endswith(extension):
            infected_files.append(directory_path)
        return infected_files

    # Case 2: It's a directory - check all items inside
    if os.path.isdir(directory_path):
        for item in os.listdir(directory_path):
            item_path = os.path.join(directory_path, item)
    ...
    # Case 3: It's neither a file nor a directory (e.g., broken path) - return empty list
    return []
...
if __name__ == "__main__":
    ...
    print("Total Infected Files in Finance:", len(find_infected_files("breach_data/Finance"))) # ???
    print("Total Infected Files in HR:", len(find_infected_files("breach_data/HR"))) # ???
